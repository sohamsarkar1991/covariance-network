...
def print_covariance(d,method=None,K_tr=100,rot=None):
    print('Covariance printing started')
    C = cov_mat(K_tr,d,method,rot)
    np.savetxt('True_cov_grid.dat',C,fmt='%.10f')
    del C
    print('Done')

def datagen_and_print(N,K,d,method=None,rot=None,M=10000,print_cov=False,K_tr=100):
    ...
    u = locations(K_tr,d)
    idx = indices(u,K,d)
    np.savetxt('indices_grid.dat',idx,fmt='%d')
        

    if print_cov:
        print_locations(K_tr,d,"True_locations_grid.dat")
        print('Covariance printing started')    
        C = cov_mat(K_tr,d,method,rot)
        np.savetxt('True_cov_grid.dat',C,fmt='%.10f')
        del C
